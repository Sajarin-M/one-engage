...
type MiddlewareResult<U extends Middleware[], M extends Middleware> = Middleware[] extends U
  ...
  return (({ use, query, params, body, result, handler }) => {
    ...
    return (async (req: any, res: any, next: (err?: unknown) => void) => {
      ...
      try {
        ...
        const output = await (handler as (...args: unknown[]) => unknown)(req, res, localNext);
